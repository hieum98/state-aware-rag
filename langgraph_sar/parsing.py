...
def extract_info_from_text(
    ...
    # Strategy 0: first balanced JSON object (handles trailing token garbage from max_tokens)
    first_obj = _extract_first_json_object(text)
    if isinstance(first_obj, dict):
        if len(text.strip()) > len(json.dumps(first_obj)) + 32:
            warn_explicit(
                "Model output contained trailing text after the first JSON object; truncated for parsing.",
                component="parsing.extract_info_from_text",
                details={"raw_chars": len(text), "json_chars": len(json.dumps(first_obj))},
            )
        for key, vtype, opt in zip(keys, value_type, optional_by_key):
            if key in first_obj:
                extracted_info[key] = _convert_value(first_obj[key], vtype)
            elif opt:
                extracted_info[key] = None
        if extracted_info and all(
            key in extracted_info for key, opt in zip(keys, optional_by_key) if not opt
        ):
    ...
    try:
        ...
        warn_explicit(
            "Full-text JSON parse failed; trying embedded JSON blocks and per-field regex.",
            component="parsing.extract_info_from_text",
