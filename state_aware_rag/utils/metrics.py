# Modified from flashrag.utils.metrics
...
class BaseMetric:
    ...
    def get_dataset_answer(self, data: datasets.Dataset) -> list:
        if 'choices' not in data.features:
            golden_answers_list = data['golden_answers']
        elif any(choice == [] for choice in data['choices']):
            golden_answers_list = data['golden_answers']
        else:
        ...
        for golden_answer in golden_answers:
            if self.is_regex:
                ...
                if match is not None:
                    score = 1.0
                    break
                predict_answer = re.compile(normalized_prediction, re.IGNORECASE)
                match = re.search(predict_answer, golden_answer)
                if match is not None:
                    score = 1.0
                    break
            else:
...
class Retrieval_Recall(BaseMetric):
    ...
    def calculate_metric(self, data):
        ...
        for doc_list, golden_answers in zip(retrieve_docs, golden_answers_list):
            ...
            for doc in doc_list:
                for golden_answer in golden_answers:
                    if normalize_text(golden_answer) in normalize_text(doc):
...
class BLEU(BaseMetric):
    ...
    def __init__(self, config):
        super().__init__(config)
        from flashrag.evaluator._bleu import Tokenizer13a
    ...
    def calculate_metric(self, data):
        from flashrag.evaluator._bleu import compute_bleu
...
class LLMJudge(BaseMetric):
    ...
    def __init__(self, config):
        ...
        metric_model_config = config.get("judge_setting", {})
        self.evaluator_agent = EvaluatorAgent(metric_model_config)

    def calculate_metric(self, data):
        question_list = data['question']
        if 'detailed_answer' in data.features:
            pred_list = data['detailed_answer']
        else:
            pred_list = data['pred']
        golden_answers_list = self.get_dataset_answer(data)
        ...
        tasks = []
        for i in tqdm.tqdm(range(0, len(question_list), 16)):
            batch_questions = question_list[i:i+16]
            batch_preds = pred_list[i:i+16]
            batch_golden_answers = golden_answers_list[i:i+16]
            ...
            responses, _, _ = asyncio.run(self.evaluator_agent.execute(instance_id=f"llm_judge_with_{i}", parameters=params_with))
            for x in responses:
                if x is None:
                    x = 0
                try:
            ...
            #     "system_answer": batch_preds,
            # }
            # responses_without_golden, _, _ = asyncio.run(self.evaluator_agent.execute(instance_id=f"llm_judge_without_{i}", parameters=params_without))
